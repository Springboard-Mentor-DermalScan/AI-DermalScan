...
def load_all_models():
    """Loads all necessary models for face detection and dermalscan."""
    try:
        # Load DermalScan Model
...
# Helper Function for Prediction and Annotation 
def process_and_predict(image_np, filename="uploaded_image"):
    """
    Processes image, detects faces, predicts signs/estimated age, and returns annotated output.
    :param image_np: numpy array of the image (from Streamlit Upload)
    :param filename: name of the uploaded file
    :return: Annotated image (BGR np array), Prediction DataFrame, Latency
    """
    ...
    for (x, y, width, height) in faces:
        ...
        # Facial Sign Prediction (DenseNet121)
    ...
    print(f"[{datetime.datetime.now().strftime('%H:%M:%S')}] Processed {filename}. Faces Detected: {len(faces)}. Latency: {latency:.2f} seconds.")
