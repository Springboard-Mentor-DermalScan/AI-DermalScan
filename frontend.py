...
def process_image(image_bytes):
    ...
    try:
        ...
        result_img, results = detect_predict_face(temp_path)
        end_time = time.time()
        processing_time = round(end_time - start_time, 2)

        if results is None or len(results) == 0:
            # No faces detected
            return image, result_img, None, None, None, "-", "-", "-", "-", "-", processing_time

        #Use first detected face for quick display
        first_face = results[0]
        condition = first_face["predicted_class"]
        age = first_face["age"]
        confidence = first_face["confidence"]
        x, y, w, h = first_face["bbox"]

        return image, result_img, results, condition, age, confidence, x, y, w, h, processing_time
...
def analyze_image(upload):
    try:
        ...
        image, result_img , results , condition , age , confidence , x, y , w , h, processing_time = process_image(image_bytes)
        ...
        # Display prediction info in table format
        with col2:
            st.markdown("### Prediction Details (All Detected Faces)")
            if results:
                rows = []
        ...
        if results:
            ...
            for res in results:
                ...
                log_entries.append({
                    ...
                    "Predicted Condition": res["predicted_class"],
                    "Confidence": res["confidence"],
                    "Predicted Age": res["age"],
                    "Processing Time (s)": processing_time
                })
            log_entry = pd.DataFrame(log_entries)
        else:
            log_entry = pd.DataFrame([{
                "Image Name": upload.name if hasattr(upload, 'name') else upload,
                "X": "-", "Y": "-", "Width": "-", "Height": "-",
                "Predicted Condition": "No Face Detected",
                "Confidence": "-",
                "Predicted Age": "-",
                "Processing Time (s)": processing_time
    }])
