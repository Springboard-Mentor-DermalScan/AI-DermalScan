...
dermalscan_model_path = "model.h5"
dermalscan_model = load_model(dermalscan_model_path)
dermalscan_classes = ['clear face', 'darkspots', 'puffy eyes', 'wrinkles']


face_cascade = cv2.CascadeClassifier(cv2.data.haarcascades + 'haarcascade_frontalface_default.xml')


with h5py.File(dermalscan_model_path, 'r') as f:
    print(list(f.keys()))

print("Model path:", dermalscan_model_path)
print("Exists:", os.path.exists(dermalscan_model_path))
...
def analyze_face(image_path):
    ...
    for (x, y, fw, fh) in faces:
        ...
        # Model prediction
        pred = dermalscan_model.predict(face_array, verbose=0)[0] # Changed from skin_model.predict
        skin_index = np.argmax(pred)
        skin_type = dermalscan_classes[skin_index]
